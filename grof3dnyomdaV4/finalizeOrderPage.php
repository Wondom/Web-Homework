...
<body>
    ...
    <div class="cityChooseDiv">
        <form action="finalizeOrderPage.php" method="POST">
            <select name="cityChoose">
                <?php
                while ($row = mysqli_fetch_array($result)) {
                    echo '<option value="' . $row['package_point_city'] . '">' . $row['package_point_city'] . ' <option>';
                }
                ?>
            </select>
            <input type="submit" id="submitCity" name="submitCity" value="Kiválasztom ezt a várost!" >
        </form><?php
        if (isset($_POST['submitCity'])) {
            $_SESSION['choosenCity'] = $_POST['cityChoose'];
            $sql = "SELECT DISTINCT package_point_address FROM package_point WHERE package_point_city LIKE '".$_POST['cityChoose']."'";
            $result = mysqli_query($conn, $sql);
            if (mysqli_num_rows($result) > 0) {
                echo '<form action=php/finalizeOrder.php method="POST"> ';
                echo '<select name="choosenAddress">';
                while($row = mysqli_fetch_array($result)){
                    echo '<option value="'.$row['package_point_address'].'"> '.$row['package_point_address'].'</option>';
                }
                echo '</select>';
                echo '<script> document.getElementById(\'submitCity\').disabled = true;</script>';
                echo '<input type="submit" name="submit" id="submit" value="Rendelés">';
            } else {
