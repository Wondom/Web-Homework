...
<html  id="html" lang="en" data-bs-theme="<?php echo $_SESSION['color'] ;?>">
    ...
    <nav class="navbar sticky-top navbar-expand-sm">
        ...
        if (isset($_SESSION['logged']) and $_SESSION['logged'] == true) {
            ...
            echo '
        <a href="logoff.php" class="btrap-icons">
        ...
        <h1>Rendelés további tudnivalói: </h1>
        <table>
            ...
            <tbody class="table-hover">
                <?php 
                    $sql = "SELECT user_name, firstName, lasName,email, total, package_point_postal_code, postal_city, package_point_address FROM orders, package_points, postal_codes WHERE orders.package_point = package_points.package_point_id AND package_points.package_point_postal_code = postal_codes.postal_code AND order_number = ".$orderNumber.";";
                    //echo $sql;
                    $result = mysqli_query($conn, $sql);
                    if (mysqli_num_rows($result) > 0) {
                        while($row = mysqli_fetch_array($result)){
                            echo '<tr><td> '.$orderNumber.' </td>
                            <td> '.$row['user_name'].' </td>
                            <td> '.$row['firstName'].' </td>
                            <td> '.$row['lasName'].' </td>
                            <td> '.$row['email'].' </td>
                            <td> '.$row['total'].' </td>
                            <td> '.$row['package_point_postal_code'].' </td>
                            <td> '.$row['postal_city'].' </td>
                            <td> '.$row['package_point_address'].' </td>
                            </tr>';
                        }
                    }   
        ...
        <h2>Ordered product</h2>
                <div class="row">
                <?php 
                    $sql = "SELECT product_name, product_count FROM orderedproducts WHERE order_number = $orderNumber";
                    $result = mysqli_query($conn, $sql);
                    if (mysqli_num_rows($result) > 0) {
                        while($row = mysqli_fetch_array($result)){
                            echo '<div class="card w-25">
                                ...
                                <div class="card-text"> 
                                    <p> number of pieces: '.$row['product_count'].'db
                                    </p>
                                </div>
                            </div>';
                        }
                ...
                <a href="completedOrder.php"><button> A rendelés elkészült!</button></a>
    </body>
</html>